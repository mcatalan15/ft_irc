...
/*void	Server::topicDisplay(string& cmd, int fd) {
	for (size_t i = 0; i < _channels.size(); i++) {
		if (cmd[1] == _channels[i].getName()) {
			// name is correct
			std::cout << "channel exists" << std::endl;
			if (isOnChan(cmd, fd)) {
				if (_channels[i].getTopic().empty()) {
					//RPL_NOTOPIC
					sendMsg(RPL_NOTOPIC(getClient(fd)->getNickname(), cmd[1]) , fd);
				} else {
					//RPL_TOPIC
					sendMsg(RPL_TOPIC(getClient(fd)->getNickname(), cmd[1], _channels[i].getTopic()), fd);
				}
			} else {
				std::cout << "is not on the channel" << std::endl;
				sendMsg(ERR_NOTONCHANNEL(getClient(fd)->getNickname(), cmd[1]), fd);
			}
		} else {
			// channel no exist
			sendMsg(ERR_NOSUCHCHANNEL(getClient(fd)->getNickname(), cmd[1]));
		}
	}
}*/
...
void	Server::topicCmd(std::vector<string>& cmd, int fd){
	...
	if (cmd.size() == 2) {
		//topicDislay(cmd[1], fd);
		for (size_t i = 0; i < _channels.size(); i++) {
			if (cmd[1] == _channels[i].getName()) {
				// name is correct
				std::cout << "channel exists" << std::endl;
				if (isOnChan(cmd[1], fd)) {
					if (_channels[i].getTopic().empty()) {
						//RPL_NOTOPIC
						return(sendMsg(RPL_NOTOPIC(getClient(fd)->getNickname(), cmd[1]) , fd));
					} else {
						//RPL_TOPIC
						sendMsg(RPL_TOPIC(getClient(fd)->getNickname(), cmd[1], _channels[i].getTopic()), fd);
						return (sendMsg(RPL_TOPICWHOTIME(getClient(fd)->getNickname(), cmd[1], getClient(fd)->getNickname(),_channels[i].getCreationTime()), fd));
					}
				} else {
					// the client is not in the channel
					return(sendMsg(ERR_NOTONCHANNEL(getClient(fd)->getNickname(), cmd[1]), fd));
				}
			} else {
				// channel no exist
				return(sendMsg(ERR_NOSUCHCHANNEL(getClient(fd)->getNickname(), cmd[1]), fd));
			}
		}
	}
		//print the topic of the channel
	if (cmd.size() > 2) {
